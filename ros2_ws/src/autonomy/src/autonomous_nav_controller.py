...
class AutonomousNavController(Node):
    ...
    def run_waypoint(self):
        ...
        if self.use_gps:
            ...
            goal_pose = self._build_pose(self.goal_lat, self.goal_lon)
    ...
    def run_search(self):
        ...
        if self.use_gps:
            ...
            # Call robot_localization /fromLL service to convert lat/lon -> map coords
            if not self.from_ll_client.wait_for_service(timeout_sec=5.0):
                self.get_logger().error("Service /fromLL not available! Cannot convert GPS goal.")
                return

            req = FromLL.Request()
            req.ll_point.latitude = self.goal_lat
            req.ll_point.longitude = self.goal_lon
            req.ll_point.altitude = 0.0

            self._publish_status("[ SEARCH ]: Converting GPS center to Map frame...")
            future = self.from_ll_client.call_async(req)
            rclpy.spin_until_future_complete(self, future)
            
            try:
                resp = future.result()
                cx, cy = resp.map_point.x, resp.map_point.y
                self._publish_status(f"[ SEARCH ]: Converted center -> ({cx:.2f}, {cy:.2f})")
    ...
    def run_demo_mode(self):
        ...
        * drive 2m forward from origin
        ...
        goal_pose = self._build_pose(2.0, 0.0, yaw=0.0)
        self.navigator.goToPose(goal_pose)
